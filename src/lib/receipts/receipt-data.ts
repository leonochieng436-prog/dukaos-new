...
type SettingsLike = { paperSize?: string | null; showBusinessLogo?: boolean | null; backgroundLogoUrl?: string | null; showBusinessAddress?: boolean | null; showBusinessContact?: boolean | null; showBranch?: boolean | null; showReceiptNumber?: boolean | null; showDate?: boolean | null; showCashier?: boolean | null; showCustomer?: boolean | null; showSku?: boolean | null; showTax?: boolean | null; showDiscount?: boolean | null; showPaymentReference?: boolean | null; footerMessage?: string | null } | null;
...
export function buildReceiptData(sale: SaleLike, settings: SettingsLike): ReceiptData {
  return {
    ...
    notes: sale.notes,
    settings: { paperSize: normalizePaperSize(settings?.paperSize), showBusinessLogo: settings?.showBusinessLogo !== false, backgroundLogoUrl: settings?.backgroundLogoUrl || null, showBusinessAddress: settings?.showBusinessAddress !== false, showBusinessContact: settings?.showBusinessContact !== false, showBranch: settings?.showBranch !== false, showReceiptNumber: settings?.showReceiptNumber !== false, showDate: settings?.showDate !== false, showCashier: settings?.showCashier !== false, showCustomer: settings?.showCustomer !== false, showSku: settings?.showSku !== false, showTax: settings?.showTax !== false && toNumber(sale.taxTotal) > 0, showDiscount: settings?.showDiscount !== false && toNumber(sale.discountTotal) > 0, showPaymentReference: settings?.showPaymentReference !== false, footerMessage: settings?.footerMessage || "Thank you for shopping with us!" },
  };
}
