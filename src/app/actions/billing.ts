...
export async function changePlan(raw: unknown): Promise<ActionResult<undefined>> {
  try {
    ...
    const subscription = await ctx.db.subscription.upsert({ where: { organizationId: ctx.organizationId }, update: { plan: input.plan, status: input.plan === "trial" ? "trialing" : "active", branchLimit: plan.branchLimit, userLimit: plan.userLimit, currentPeriodEnd: periodEnd, trialEndsAt }, create: { organizationId: ctx.organizationId, plan: input.plan, status: input.plan === "trial" ? "trialing" : "active", branchLimit: plan.branchLimit, userLimit: plan.userLimit, currentPeriodEnd: periodEnd, trialEndsAt } });
