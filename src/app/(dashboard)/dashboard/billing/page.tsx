...
import { computeEffectiveSubscriptionStatus } from "@/lib/billing";
...
export default async function BillingPage() {
  ...
  return <div className="max-w-2xl space-y-6"><div><h1 className="text-lg font-semibold">Subscriptions & billing</h1><p className="text-sm text-muted-foreground">Manage plan status and organization usage limits.</p></div><Card><CardHeader><CardTitle>Current plan</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm">Plan: <strong>{current.plan}</strong> · Status: <strong>{effectiveStatus}</strong></p><p className="text-sm text-muted-foreground">Up to {current.branchLimit} branches and {current.userLimit} users.</p><p className="text-sm text-muted-foreground">Renews: {current.currentPeriodEnd ? current.currentPeriodEnd.toLocaleDateString() : "Not scheduled"}</p><PlanForm currentPlan={current.plan} /></CardContent></Card></div>;
}
