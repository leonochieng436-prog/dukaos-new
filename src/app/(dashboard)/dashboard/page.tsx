...
export default async function DashboardPage({
  ...
  const [organization, branches, periodSales, previousSales, products, recentSales, periodReturns, creditCustomers] = await Promise.all([
    ...
    ctx.db.sale.findMany({ where: { ...branchWhere, status: "COMPLETED", createdAt: { gte: periodStart } }, select: { id: true, total: true, cogsTotal: true, discountTotal: true, createdAt: true, items: { select: { productNameSnapshot: true, quantity: true, total: true } }, payments: { where: { status: "CONFIRMED" }, select: { method: true, amount: true } }, customerId: true } }),
  ...
  const servedCustomers = new Set(periodSales.map((sale) => sale.customerId ?? `walk-in:${sale.id}`)).size;
  ...
  const lowStock = products.filter((product) => product.variants.some((variant) => {
    ...
    return quantity.gt(0) && (quantity.lessThan(5) || quantity.lessThanOrEqualTo(String(variant.reorderLevel)));
  })).length;
  ...
  const outOfStockProducts = products.filter((product) => product.variants.length > 0 && product.variants.every((variant) => variant.inventoryItems.reduce((total, item) => total.plus(String(item.quantity)), new Decimal(0)).isZero()));
    ...
    <div className="dashboard-home space-y-6">
      <section className="dashboard-hero flex flex-col justify-between gap-6 rounded-[var(--radius-lg)] px-6 py-7 text-white shadow-[0_14px_30px_rgba(15,123,108,0.14)] sm:flex-row sm:items-end sm:px-8">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          <Link href="/dashboard/pos" className="inline-flex h-10 items-center justify-center gap-2 rounded-[var(--radius-sm)] bg-white px-4 text-sm font-semibold text-primary transition-all duration-150 hover:-translate-y-0.5 hover:shadow-[0_8px_18px_rgba(15,123,108,0.16)]">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <Card><CardContent className="p-5"><div className="flex items-center justify-between"><div><p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-primary">Item analytics</p><h2 className="mt-1 text-lg font-semibold">Top products today</h2></div><Link href="/dashboard/reports" className="text-[12px] font-semibold text-primary hover:underline">View reports</Link></div>{topProductsList.length === 0 ? <p className="mt-5 text-sm text-muted-foreground">No products sold today.</p> : <div className="mt-4 divide-y divide-border">{topProductsList.map(([name, quantity], index) => <div key={name} className="flex items-center justify-between gap-4 py-3 first:pt-0"><div className="flex min-w-0 items-center gap-3"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-primary-tint text-[11px] font-semibold text-primary">{String(index + 1).padStart(2, "0")}</span><span className="truncate text-sm font-medium">{name}</span></div><span className="shrink-0 font-tabular text-sm font-semibold">{quantity.toFixed(0)} sold</span></div>)}</div>}<div className="mt-5 border-t border-border pt-4"><div className="flex items-center justify-between"><div><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-danger">Out of stock</p><p className="mt-1 text-[12px] text-muted-foreground">Items unavailable for sale</p></div><Link href="/dashboard/inventory" className="font-tabular text-sm font-semibold text-danger hover:underline">{outOfStock}</Link></div>{outOfStockProducts.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{outOfStockProducts.slice(0, 5).map((product) => <Link key={product.id} href="/dashboard/inventory" className="rounded-[var(--radius-sm)] bg-danger-tint px-2.5 py-1.5 text-[12px] font-medium text-danger hover:underline">{product.name}</Link>)}{outOfStockProducts.length > 5 && <Link href="/dashboard/inventory" className="rounded-[var(--radius-sm)] bg-surface-muted px-2.5 py-1.5 text-[12px] text-muted-foreground hover:underline">+{outOfStockProducts.length - 5} more</Link>}</div>}</div></CardContent></Card>
